...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            userID = env.UUID || env.uuid || env.u || env.U || env.PASSWORD || env.pswd || userID;
            ...
            proxyIP = env.PROXYIP || env.p || env.P ||  || env.proxyip || proxyIP;
            ...
            if (socks5Address) {
                try {
                    ...
                    请求CF反代IP = env.p || 'false';
                    enableSocks = true;
                } catch (err) {
                    let e = err;
                    console.log(e.toString());
                    请求CF反代IP = env.p || !proxyIP ? 'true' : 'false';
                    enableSocks = false;
                }
            } else {
                请求CF反代IP = env.p || !proxyIP ? 'true' : 'false';
